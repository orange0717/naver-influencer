...
## 사이드바 (변경 금지)
`src/lib/sidebar-nav.ts` 의 `SIDEBAR_GROUPS` 는 **고정값**이다(2026-09-03 확정).
- 다른 작업의 곁다리로 항목 추가·삭제·이동, 라벨·그룹 구성 변경을 하지 말 것
- 새 페이지를 만들어도 메뉴에 자동 등록하지 않는다 — 노출은 그때마다 별도 지시를 받는다
- 현행 구조: `대시보드 / 네이버 데이터 / 콘텐츠 도구 / 구글` (2026-09-02 v2.3 재편은 되돌림)
