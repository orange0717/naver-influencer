/**
 * 패션 공식 인플루언서 순위 임포트 (JSON 파일 기반)
 *
 * 사용법: node scripts/import-fashion-ranks.mjs
 *
 * fashion-ranks.json에서 (name, rank) 배열을 읽어
 * influencers 테이블의 official_naver_rank, official_rank_category를 업데이트
 */
...
// JSON 파일 읽기
const jsonPath = resolve(import.meta.dirname, 'fashion-ranks.json');
let parsed;
try {
  const raw = readFileSync(jsonPath, 'utf-8');
  parsed = JSON.parse(raw);
} catch (err) {
  console.error('JSON 파일을 읽을 수 없습니다:', jsonPath);
...
for (const { name, rank } of parsed) {
  ...
  if (inf) {
    updates.push({
      ...
      official_rank_category: CATEGORY,
    });
    matched++;
  } else {
