...
describe('createRateLimiter', () => {
  ...
  it('제한 초과 시 true 반환', () => {
    ...
    expect(limiter.check('user1')).toBe(false);
    expect(limiter.check('user1')).toBe(false);
    expect(limiter.check('user1')).toBe(true); // 초과
  });

  it('서로 다른 키는 독립적으로 카운트', () => {
    const limiter = createRateLimiter({ limit: 1, windowMs: 60000 });

    expect(limiter.check('user1')).toBe(false);
    expect(limiter.check('user2')).toBe(false);
    expect(limiter.check('user1')).toBe(true); // user1만 초과
    expect(limiter.check('user2')).toBe(true); // user2도 초과
  });
