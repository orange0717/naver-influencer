...
// 블로거용 이벤트 생성
export function generateBloggerEvents(results: { keyword: string; rank: number | null; prevRank: number | null }[]): ActivityEvent[] {
  const events: ActivityEvent[] = [];
  const now = new Date().toISOString();

  for (const r of results) {
    if (r.rank === null || r.prevRank === null) continue;
    const change = r.prevRank - r.rank;

    if (change > 0) {
      if (r.rank <= 5 && r.prevRank > 5) {
        events.push({
          id: `top5-${r.keyword}`,
          type: 'new_top3',
          keyword: r.keyword,
          from_rank: r.prevRank,
          to_rank: r.rank,
          change,
          message: `${r.keyword} 키워드가 TOP 5에 진입! (${r.prevRank}위 → ${r.rank}위)`,
          timestamp: now,
        });
      } else {
        events.push({
          ...
          to_rank: r.rank,
          change,
          message: `${r.keyword} 순위 ${change}단계 상승 (${r.prevRank}위 → ${r.rank}위)`,
          timestamp: now,
        });
      }
    } else if (change < 0) {
      events.push({
        ...
        to_rank: r.rank,
        change,
        message: `${r.keyword} 순위 ${Math.abs(change)}단계 하락 (${r.prevRank}위 → ${r.rank}위)`,
        timestamp: now,
      });
    }
  }
