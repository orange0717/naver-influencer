  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      const { error: oauthError } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${window.location.origin}/auth/callback?next=/my/blogger`,
        },
      });
      if (oauthError) {
          ...
          <div className="space-y-4 animate-fade-in-up">
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-[10px] uppercase">
                <span className="bg-surface px-2 text-dim">또는</span>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-3 bg-white hover:bg-gray-50 text-gray-700 font-bold rounded-xl transition cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed border border-border flex items-center justify-center gap-2"
            >
              {googleLoading ? (
                <span className="flex items-center justify-center gap-2">
