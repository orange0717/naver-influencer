  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <AnimatedStatCard label="누락율" value={blogScoreCalc.hasData && blogScoreCalc.totalKeywords > 0 ? Math.round(((blogScoreCalc.totalKeywords - Math.max(blogScoreCalc.viewExposed, blogScoreCalc.blogExposed)) / blogScoreCalc.totalKeywords) * 100) : 0} suffix="%" placeholder={blogScoreCalc.hasData ? undefined : '—'} description={blogScoreCalc.hasData ? `${blogScoreCalc.totalKeywords}개 중 ${blogScoreCalc.totalKeywords - Math.max(blogScoreCalc.viewExposed, blogScoreCalc.blogExposed)}개 누락` : '키워드순위에서 확인 필요'} icon={<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>} color={(() => { if (!blogScoreCalc.hasData || blogScoreCalc.totalKeywords === 0) return 'dim'; const rate = ((blogScoreCalc.totalKeywords - Math.max(blogScoreCalc.viewExposed, blogScoreCalc.blogExposed)) / blogScoreCalc.totalKeywords) * 100; return rate <= 30 ? 'up' : rate <= 60 ? 'accent' : 'down'; })()} delay={200} />
      </div>
