...
/**
 * GET: 마운트 시 (블로그 단위) 포스트별 영속화된 대표 키워드 복원.
 * post_representative_keywords(migration-130)는 blog_id 기준 공용 테이블이라 크롤링 없이 즉시 조회 가능 —
 * 아직 추출되지 않은 포스트는 결과에서 빠지며, 클라이언트가 /api/blog/representative-keywords로 개별 추출을 트리거한다.
 */
export async function GET(request: NextRequest) {
  ...
  const { data, error } = await supabase
    ...
    .select('post_id, representative_keyword, keyword_source, extracted_at')
    .eq('blog_id', blogId);

  if (error) return NextResponse.json({ error: '조회에 실패했습니다.' }, { status: 500 });

  const results: Record<string, { keyword: string | null; source: string | null; extractedAt: string | null }> = {};
