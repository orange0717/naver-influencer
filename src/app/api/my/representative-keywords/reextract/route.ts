...
import { extractKeywordCandidates, auditStoredKeyword } from '@/lib/keyword-candidates';
import { upsertRepresentativeRows } from '@/lib/post-keyword-extractor';
 ...
 * 의심/미추출 대상만 제목 기반 규칙으로 재추출해 저장한다(네이버 무호출).
...
export async function POST(request: NextRequest) {
  ...
  for (const r of rows) {
    ...
    const extracted = extractKeywordCandidates({ title: r.post_title });
    const keywords = extracted.primary ? [extracted.primary, ...extracted.secondaries] : [];
