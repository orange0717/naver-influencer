...
export function DashboardCardIcon({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <div className={`w-8 h-8 rounded-lg bg-accent/15 flex items-center justify-center shrink-0 ${className}`.trim()}>
      {children}
    </div>
  );
}
...
export default function DashboardCard({
  ...
  return (
    <div
      id={id}
      className={`bg-surface rounded-2xl border border-border shadow-xs ${padding === 'md' ? 'p-6' : ''} ${className}`.trim()}
    >
      {hasHeader && (
