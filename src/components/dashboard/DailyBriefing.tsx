...
export default function DailyBriefing({
  ...
  // 포스팅 플래너 작성중 개수 fetch
  useEffect(() => {
    fetch('/api/my/keyword-plans')
      .then(r => r.ok ? r.json() : null)
      .then(data => {
        if (data?.plans) {
          setPlanCount(data.plans.filter((p: { status: string }) => p.status === 'writing').length);
        }
      })
      .catch(() => {});
  }, []);
  ...
  if (!hasAnyChange && !bestUp && !worstDown && planCount === 0) return null;
  ...
  return (
    <div className="bg-gradient-to-br from-surface to-accent/[0.04] rounded-2xl border border-accent/15 shadow-[0_1px_3px_rgba(0,0,0,0.04)] p-4">
      {/* 헤더 */}
