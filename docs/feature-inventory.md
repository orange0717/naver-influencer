...
3. 「MY 블로그」·「MY 키워드 챌린지」의 정체 (§3.6 잔존 2건) — 추측 금지, 오렌지 확인 필요
4. 유료 판매 중인데 메뉴가 없는 4건(§2-A)을 새 구조 어디에 넣을 것인가
...
| # | 항목 | 왜 막히나 |
|---|---|---|
| A | 3·6·9개월 금액의 **100원 반올림**이 의도된 것인가 | §7. 의도라면 지시서 §7.1 기준값 쪽을 고쳐야 한다 |
| B | **「MY 블로그」의 정체** | `SubscribeClient.tsx` 두 곳에만 있는 문자열. 대응 화면 불명 |
| C | §6.5 「합산 쿼터 제거」 지시를 취소할 것인가 | §4-1. 2026-09-01 「쿼터 유지」 결정과 정면 충돌 |
| D | 유료로 파는데 메뉴가 없는 4건(§0)을 사이드바에 올릴 것인가 | Phase 1 제안서 §2-A |
