# Phase 0 — 현황 스캔 (지시서 v2.1 §0·§2·§3 + v2.2 신규 실측 + v2.3 반영)

> 작성일 2026-09-01 / 대상 커밋 `de4ead69` / 방식: 코드 실측. 추정한 곳은 「추정」으로 표시했다.
> **v2.2 가 새로 요구한 실측 5건은 §8 에 따로 모았다** (kind 판별 · MY 한정성 · 롱폼 산출물 ·
> 금액 반올림 판정 · 쿼터 관측). §7 표의 A·B·C·D 는 그 결과로 해소됐고, **E·F 가 새로 열렸다.**
>
> 🚨 **v2.3 Phase 1.5 로 §7 말미의 「§9-8~12 전부 해소」 서술을 정정했다.** §9-8·§9-11 은
> 미해소이고 §9-10 은 부분 해소다. 대조표는 `docs/plan-spec-diff.md`.
...
### §3.6 이용권 항목 8건 대응 메뉴 불명 → ✅ **8건 전부 해소** (v2.3 §5 확정으로 종결)
| 이용권 표기 | 실측 |
|---|---|
...
| **MY 블로그** | ✅ **`/dashboard` 확정** (v2.3 §5, 확신도 확실). 문자열 자체는 `SubscribeClient.tsx:244,418` 두 곳뿐이고 코드에 대응 라벨이 없지만, 「MY 키워드순위」가 유료로 별도 판매되므로 「블로그 카테고리 전체」 해석과 공존 불가 → `/dashboard` 단일 화면이 유일하게 모순 없는 독법. **Phase 1.5 에서 해당 행 불변 확인 → 근거 유효** |
| **MY 키워드 챌린지** | ✅ **되살리지 않음으로 종결** (v2.3 §4). 이용권 페이지에서 이미 삭제된 표기이고, 가리키던 `/keywords` 는 전체 목록이라 MY 규칙 위반이기도 하다. 사이드바 통일명은 MY 없이 **「키워드 챌린지」** |
...
3. ~~「MY 블로그」·「MY 키워드 챌린지」의 정체~~ → **둘 다 해소.** 「MY 키워드 챌린지」는 §8-2
   (본인 한정이 아니라 MY 를 붙이면 안 되는 이름, 이용권에서 이미 삭제 → 되살리지 않음),
   「MY 블로그」는 **v2.3 §5 로 `/dashboard` 확정**(§9 대조에서 근거 유효 확인)
...
| B | **「MY 블로그」의 정체** | ✅ **v2.3 §5 로 `/dashboard` 확정.** Phase 1.5 에서 무효화 조건(「MY 키워드순위」 플랜 변경)이 발동하지 않음을 확인 → 근거 유효 |
...
| E | 🔴 **이용권 정본 변경분 15건** | ⏳ **Phase 1.5 결과, 정지·승인 대기.** `docs/plan-spec-diff.md`. 승인 전 Phase 2 매핑 착수 금지 |
| F | 🆕 **판매 카드에서 빠진 유료 3건** | ⏳ 키워드 추천(AI)·글감 찾기(AI)·릴스·쇼츠 분석(AI)이 비교표엔 있고 인플루언서 카드엔 없다. §9 적재인가 이번 범위인가 |

🚨 **아래 서술은 Phase 1.5 실측으로 정정됐다.** 이전 판(v2.1)에서 「§9-8~§9-12 전부 해소, 다시
올리지 말 것」이라고 적었으나, 그 판단은 **stale 한 정본을 근거로 한 것**이었고 번호 대응도
어긋나 있었다(지시서 §9-8 은 경쟁자 분석이 아니라 **CTA 문구**다). v2.3 지시로 재점검한 결과:

| 번호 | 항목 | Phase 1.5 재점검 |
|---|---|---|
| §9-8 | CTA 문구 —「무료로 시작하기」 3종 동일 | 🔴 **미해소.** `:241`·`:283`·`:324` 세 카드 모두 같은 문구·같은 목적지(`/auth/signup`). 유료 카드를 눌러도 무료 가입으로 간다 |
| §9-9 | 경쟁자 분석 중복 표기 (카드 1일1회 vs 표 1일3회) | ✅ **해소.** 카드에서 항목이 빠지고 표는 `–` 로 통일. 남은 「무제한」 한 쌍은 카드·표 일치 |
| §9-10 | 다운로드 한도 차등 | ⚠️ **부분 해소.** 포스팅이 「인플 무제한」→「1회 500건」이 되어 키워드와 한도 일치. 다만 **「1회」의 단위(요청당/일당)를 페이지가 정의하지 않는다** → Phase 3 코드 대조로 이월 |
| §9-11 | 맞춤법 검사 「데모 제외」의 데모 정의 | 🔴 **미해소.** 맞춤법(`:292`,`:511`)·글 피드백(`:523`)에 꼬리표만 있고 데모의 정의·범위·횟수가 페이지 어디에도 없다 |
| §9-12 | 합산 쿼터 | ✅ 관측 전용으로 확정(범위 밖) → §8-5 |
| §9-13 | 등급 키 명칭 | ✅ `PlanKey` 3종으로 이미 정리됨 |

**교훈**: 「해소됐으니 다시 올리지 말 것」이라고 못 박을 때는 그 근거가 현재 화면인지부터 볼 것.
당시 근거였던 `plan-spec-source.md` 는 이미 내 커밋(`3d5a7bef`·`de4ead69`)에 의해 stale 이었다.
