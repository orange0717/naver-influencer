...
3. **⑤ 에서 「비교표에만 있는 유료 3건」(키워드 추천 · 글감 찾기 · 릴스·쇼츠 분석)은 카드에 넣지
   ...
   → **2026-09-02 오렌지 승인으로 해소**. §8 참조.

## §8 카드↔비교표 3건 해소 (2026-09-02)

§7 의 3번에서 남겨 뒀던 건이다. 셋 다 **제품이 온전히 살아 있는 정상 판매 기능**임을 재확인했다.

| 항목 | `plans.ts` | 사이드바 | 화면 가드 | API 가드 | 비교표 | 카드(전) |
|---|---|---|---|---|---|---|
| 키워드 추천 (AI) | INFLUENCER | ✓ | `checkFeaturePage` | `requireFeature` | ✓ | 없음 |
| 글감 찾기 (AI) | INFLUENCER | ✓ | `checkFeaturePage` | `requireFeature` | ✓ | 없음 |
| 릴스·쇼츠 분석 (AI) | INFLUENCER | ✓ | `checkFeaturePage` | `requireFeature` | ✓ | 없음 |

「블로거 순위」(⑥)와 정반대 방향의 불일치다 — 그쪽은 **안내만 있고 제품이 없었고**, 이 3건은
**제품이 있는데 카드가 안 팔았다**. 게다가 `faq-data.ts:52,66` 과 `intro` 소개 페이지는 이미 셋을
광고하고 있었다. 인플루언서 카드에 3줄 추가(16 → 19개), 비교표는 이미 ✓ 라 수정 없음.

배치는 사이드바의 짝을 따랐다 — 대량 키워드 조회 옆에 키워드 추천, 롱폼 분석 옆에 릴스·쇼츠 분석.

### 남은 것

- **HTTP 실측** — 오렌지가 무료·예비 인플루언서·인플루언서 **테스트 계정 3개를 발급**해 주시기로 했다.
  받는 대로 등급별 200/403 을 실측한다. 그전까지 ①②③ 은 **코드 경로상** 막혔/열렸다는 뜻이지
  「실제로 403/200 이 떨어지는 것을 봤다」가 아니다.
